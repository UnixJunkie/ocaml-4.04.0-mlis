...
(** Ripemd128.unsafe_update_substring ctx s ~pos ~len updates the context
...
(** Ripemd128.update_substring ctx s ofs len updates the context with the
...
(** Ripemd128.substring s ofs len returns the digest of the substring of
...
(** If len is nonnegative, Ripemd128.channel ic len reads len characters from
channel ic and returns their digest, or raises End_of_file if end-of-file is
reached before len characters are read. If len is negative, Ripemd128.channel ic
