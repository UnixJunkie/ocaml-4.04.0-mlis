...
(** Sha3_512.unsafe_update_substring ctx s ~pos ~len updates the context
...
(** Sha3_512.update_substring ctx s ofs len updates the context with the
...
(** Sha3_512.update_string ctx s updates the context with s. *)
...
(** Sha3_512.substring s ofs len returns the digest of the substring of
...
(** If len is nonnegative, Sha3_512.channel ic len reads len characters from
channel ic and returns their digest, or raises End_of_file if end-of-file is
reached before len characters are read. If len is negative, Sha3_512.channel ic
